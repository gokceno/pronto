...
const StickyAudioPlayer = ({ songName, name, clickcount, votes, imgSrc }) => {
  ...
  const { player, togglePlayPause, isPlaying } = usePlayer();
      ...
      <div className="flex items-center">
        <div className="w-16 h-16 rounded-xl flex items-center">
          {player.imgSrc ? (
            ...
            >
              {player.name ? formatStationName(player.name) : ""}
            </div>
          )}
        </div>
        <div className="flex items-center w-full justify-between">
          <div className={`flex flex-col pr-4 pl-3`}>
            ...
            <div className={`text-xs text-[#ffffffcc]`}>
              {player.clickcount} {t("listeningCount")} • {player.votes}{" "}
              {t("likes")}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 ml-4">
            <button
              className="w-10 h-10 flex items-center justify-center bg-[#ffffff29] hover:bg-[#167AFE] rounded-full text-white"
