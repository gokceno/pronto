...
import { HeartIcon, Share1Icon, TrashIcon, Pencil1Icon } from "@radix-ui/react-icons";
import { generateLocalizedRoute } from "../../utils/generate-route";
import { Link } from "@remix-run/react";
import { useTranslation } from "react-i18next";

export default function ListContextMenu({locale, listId="", onDelete, onShare}) {
  const { t } = useTranslation();

  return (
    <div className="w-[14.0625rem] h-[15rem] flex p-4 gap-3 bg-white flex-col shadow-lg rounded-xl z-50">
      ...
      <div className="flex flex-col w-full gap-2">
        <button className='flex flex-col w-full h-8 py-1 px-2 gap-2 hover:bg-[#E8F2FF] hover:rounded-lg transition-all'>
            ...
            <img
                ...
                className="w-6 h-6"
            />
            <Link to={generateLocalizedRoute(locale, `/details/music-list/${encodeURIComponent(listId)}`)}>
        ...
        <Link
          to={generateLocalizedRoute(locale, `/details/music-list/${encodeURIComponent(listId)}`)}
